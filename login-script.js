...
const supabaseLOGIN = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
...
function setLoadingLogin(ativo) {
    ...
    if (ativo) {
        ...
        btn.innerHTML = `Acessar Sistema <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4 10h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`;
        btn.style.opacity = '1';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    style.textContent = `@keyframes shake{0%,100%{transform:translateX(0)}10%,30%,50%,70%,90%{transform:translateX(-8px)}20%,40%,60%,80%{transform:translateX(8px)}}`;
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            const { data: usuario, error } = await supabaseLOGIN
                ...
                .eq('nome', nome).eq('senha', senha).maybeSingle();
